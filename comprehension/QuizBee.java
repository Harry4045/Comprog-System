...
public class QuizBee {
    ...
    public void writeInput() {
        try {
            ...
            do {
                ...
                // Otherwise, if the user input is invalid
                else {
    ...
    public void loadUsers() {
        try (BufferedReader reader = new BufferedReader(new FileReader("C:\\Users\\harry\\Downloads\\Comprog-System-main\\Comprog-System-main\\comprehension\\words.txt"))) {
            String line;
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader("leaderboard.txt"))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length == 2) {
                    ...
                    leaderboard.put(name, userScore);
                }
            }
        } catch (IOException e) {
            System.out.println("Error reading leaderboard from leaderboard.txt: " + e.getMessage());
        }
    }
    ...
    public void saveLeaderboard() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("leaderboard.txt"))) {
            ...
            System.out.println("Error writing leaderboard to leaderboard.txt: " + e.getMessage());
        }
    }
    ...
    public void startQuiz() {
        ...
        if (leaderboard.isEmpty()) {
            System.out.println("No users found in words.txt.");
            return;
        }

        // Retrieve the first user from words.txt as currentUser
        currentUser = leaderboard.keySet().iterator().next(); 
